...
"""
    with open(sys.argv[1], "r") as ifi:
        for line in ifi:
            line = line.strip()
            tokens = line.split()
            assigned_ref = tokens[0].split("_")[0][0]
            real = tokens[1].split("_")[0][0]
            unclassified = "FAIL" in line or tokens[2] == "0"
            if unclassified:
                n_unclassified += 1
                uncl_d[real] += 1
    ...
    for i in correct_d:
        print ("Correct,", i, ":", correct_d[i])
        if i in uncl_d:
            print ("Unclassified,", i, ":", uncl_d[i])
        if i in wrong_d:
            print ("Wrong,", i, ":", wrong_d[i])
